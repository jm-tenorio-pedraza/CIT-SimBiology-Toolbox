function [x, p_x,accept,pCR,stepSize,J, n_id] = par_H_dream_original(X0,prior,likelihood,N,T,d,varargin)
% Inputs
...
x=nan(N,d,T); p_x = nan(T,N);                               % Preallocate chains and density
Xp = nan(d,N); p_Xp = nan(N,1);p_X = nan(N,1);
CR = (1:n_CR)/n_CR;                 % Crossover values and select. prob.
R = nan(N,N-1);
for i=1:N, R(i, 1:N-1) = setdiff(1:N,i); end                % R-matrix: index of chains for DE

X = X0; % Nxd                                                     % Create initial  population
...
for t = 2:T
    ...
    parfor i = 1:N
        ...
        if isinf(p_Xp_i) || ~isreal(p_Xp_i) || isnan(p_Xp_i)
           ...
           if U(t,i)<=min(r,0)
               p_X(i,1) = propL;
               X(i,popParamIndx) = prop_p(popParamIndx);
    ...
    if ~isempty(indivParamIndx)
        ...
        parfor i = 1:N
            ...
            p_Xp_i= prior(prop_p);
            if isinf(p_Xp_i) || ~isreal(p_Xp_i) || isnan(p_Xp_i)
                dX(i,indivParamIndx) = 0;
            else
                propL = likelihood(prop_p)+p_Xp_i;                        % Compute density of ith proposal
                r = propL-p_X(i,1);
                if U(t,i)<=min(r,0)
                    p_X(i,1) = propL;
                    X(i,indivParamIndx) = prop_p(indivParamIndx);
                    accept(i,1)=accept(i,1)+1;
                else
                    dX(i,indivParamIndx) = 0;
                end
...
for i=1:N 
    ...
    [X, p_X] = check(X, mean((p_x(ceil(t/2):t,1:N))),p_X,'N', N);       % Outlier detection and correction
    
    progress((t-1)/T,mean(Xp)',(mean(accept)/(t)))            % Print out progress status
    if mod(t,20)==0
    plot(p_x)
    hold on
