function [x, p_x, stepSize, J, n_id,accept,Z] = par_dream_zs(Z0,prior, likelihood, N, T,d, varargin)
par = inputParser;
...
% if ~isempty(H)
%     IndivParamsIndx = ismember(1:d, [H.CellParams.Index H.IndividualParams.Index H.RespParams.Index]);
...
%     walkerIndx = randsample([1:N], N,false);
    ...
    if zprob==0
        ...
        z = rand(N, d);
        A = (z < CR(id)');
%         A(walkerIndx(1),:) = (IndivParamsIndx);
%         A(walkerIndx(2:N),:) = A(walkerIndx(2:N),:).*PopParamsIndx;
        d_star = sum(A,2);
        if any(d_star == 0)
