function [models,logP_x]=gwmcmc_edit(minit,logPfuns,mccount,varargin)
...
M=size(minit,1); % number of model parameters
if size(minit,2)==1
...
if isoctave
    ...
    p.addParameter('plotLogL',true);
    p.parse(varargin{:});
end
p=p.Results;


Nwalkers=size(minit,2); % Number of walkers per step
...
T=ceil(mccount/p.ThinChain/Nwalkers); %number of samples drawn from each walker
mccount=(T-1)*p.ThinChain+1;

models=nan(M,Nwalkers,T); %pre-allocate output matrix

models(:,:,1)=minit; % Allocate the first sample of walkers

if ~iscell(logPfuns)
    logPfuns={logPfuns};
end

NPfun=numel(logPfuns); % Number of functions
logP_x = nan(T,Nwalkers);
%calculate logP state initial pos of walkers
logP=nan(NPfun,Nwalkers,1); 
for wix=1:Nwalkers
    for fix=1:NPfun
...
for row=1:T
    for jj=1:p.ThinChain
        ...
        progress((row-1+jj/p.ThinChain)/T,curm,sum(reject)/totcount)
    end
    models(:,:,row)=curm;
    logP_x(row,:)=sum(curlogP,1);

    %progress bar
    if mod(row,20)==0 && p.plotLogL
        plot(logP_x)
        hold on
        plot(mean(logP_x), 'LineWidth', 4)
        ylim([quantile(logP_x(row,:), 0.02) quantile(logP_x(row,:), 0.97)])
    end

end
progress(1,0,0);
if p.BurnIn>0
    crop=ceil(T*p.BurnIn);
