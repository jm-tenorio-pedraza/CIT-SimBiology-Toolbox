...
% Define outputs
observables={'ID_g_Blood_antiPDL1_Id' 'ID_g_Blood_antiPDL1_Io' 'ID_ml_antiPDL1_Blood_free'...
    'ID_g_Tumor_antiPDL1_Id' 'ID_g_Tumor_antiPDL1_Io' 'ID_ml_antiPDL1_Tumor_free'  };
if ispc
...
[PI,PI.u]=getDataSets(dataset_file_ext, 'subsetVariables', {'Blood_antiPDL1__ID_g_'...
    'Blood_antiPDL1_Iodine__ID_g_'  'Blood_antiPDL1_free__ID_g_'  ...
    'Tumor_antiPDL1__ID_g_' 'Tumor_antiPDL1_Iodine__ID_g_' 'Tumor_antiPDL1_free__ID_g_'...
      });

PI.u = PI.u(:,1);
PI.variableUnits={'%ID/g'  '%ID/g' '%ID/g' '%ID/g' '%ID/g' '%ID/g'};
PI.observablesPlot = {'Blood Serum antiPDL1_{Total}' 'Blood Serum antiPDL1_Iodine_{Total}' 'Blood Serum antiPDL1_{Free}'...
    'Tumor antiPDL1_{Total}'  'Tumor antiPDL1_Iodine_{Total}' 'Tumor antiPDL1_{Free}' };
PI.observablesFields = {'Blood_Serum_Total', 'Blood_Serum_Total_Iodine', 'Blood_Serum_Free',...
    'Tumor_Total', 'Tumor_Total_Iodine','Tumor_Free'};
dose = {'Blood.antiPDL1'};
PI.normIndx = [];
...
%% 
sim=createSimFunction(model,parameters,observables, dose,variants(6),...
    'UseParallel', false);
...
%% Save results
save('PI_PK_ThreeComp4_9_TMDD_8.mat', 'PI')
%%
load(strjoin({cd 'PI_PK_ThreeComp4_9_TMDD_8.mat'},'/'))
%%
%% save MCMC results
N_i='7';
