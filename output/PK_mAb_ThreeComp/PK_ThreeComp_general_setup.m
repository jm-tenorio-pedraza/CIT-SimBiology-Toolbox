...
cd('/Users/migueltenorio/Documents/GitHub/CIT-SimBiology-Toolbox/output/PK_mAb_ThreeComp/PI')
sensitivity = false;
...
exclude_parameters = {'k12' 'k21' 'k23' 'k32' 'ID_g_Tumor'...
        'T2B'  'ID_g_Blood' 'ka_IP', 'ID','ke_Central'};
...
% Generating PI
alpha = [repelem(0.01, length([H.IndividualParams(:).OmegaIndex]),1);...
    repelem(0.01, length(setdiff(H.SigmaParams, [H.IndividualParams(:).OmegaIndex])),1)];
beta = [repelem(0.1, length([H.IndividualParams(:).OmegaIndex]),1);...
    repelem(0.01, length(setdiff(H.SigmaParams, [H.IndividualParams(:).OmegaIndex])),1)];
...
paramNames = ['\eta_{Blood}' '\eta_{Tumor}' 'ke_{Central}' 'Q12' 'Q23'...
    'ka_{Central}' 'Peripheral' {PI.par([H.IndividualParams(:).Index]).name}, '\omega_{Blood}',...
