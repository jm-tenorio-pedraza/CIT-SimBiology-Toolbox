...
cim_col_indx = [3 5 6];
icb_col_indx = [PI_ICB.PI.H.PopulationParams  PI_ICB.PI.H.SigmaParams(end)];
...
sigmaSamples = [PI_ICB.PI.postSamples(cim_row_indx, PI_ICB.PI.H.SigmaParams(4))...
    PI_CIM.PI.postSamples(cim_row_indx, PI_CIM.PI.H.SigmaParams(4))];
...
% Calculate SLD and response %
for i=1:4
    PI2.output(i).SLD = ((PI2.output(i).TV./(Theta2(:,end)*0.00153)).^(1/3))*100-100;
    ...
    PI2.output(i).SLD_sigma = ((PI2.output(i).TV_sigma./(Theta2(:,end)*0.00153)).^(1/3))*100-100;
...
surv = table(groups', log(2)*[sum(T2(1:5e3))/sum(~censor2(1:5e3)); sum(T2(5e3+1:1e4))/sum(~censor2(5e3+1:1e4));...
    sum(T2(1e4+1:1.5e4))/sum(~censor2(1e4+1:1.5e4)); sum(T2(1.5e4+1:2e4))/sum(~censor2(1.5e4+1:2e4))]/30);
surv{:,3} = log(2)./exp(log(1./(surv{:,2}/log(2)))+1.96*[sum(~censor2(1:5e3));sum(~censor2(5e3+1:1e4));sum(~censor2(1e4+1:1.5e4));sum(~censor2(1.5e4:2e4))].^(-1/2));
surv{:,4} = log(2)./exp(log(1./(surv{:,2}/log(2)))-1.96*[sum(~censor2(1:5e3));sum(~censor2(5e3+1:1e4));sum(~censor2(1e4+1:1.5e4));sum(~censor2(1.5e4:2e4))].^(-1/2));
surv.Properties.VariableNames = {'Group', 'Median_PFS', 'LB', 'UB'};

ORR2 = cell2mat(arrayfun(@(x)mean(x.Response), PI2.output,'UniformOutput', false));
ORR2_SE = sqrt(ORR2.*(1-ORR2)/size(Theta2,1));
ORR2_CI_UB = round(ORR2+1.96*ORR2_SE,3);
ORR2_CI_LB = round(ORR2-1.96*ORR2_SE,3);

orr2 = table({'PD'; 'SD'; 'PR'; 'CR'});
for i=1:4
    indx = (i-1)*2+2;
    orr2{:,indx} = ORR2(i,:)';
    orr2{:,indx+1} = [cellfun(@num2str, num2cell(ORR2_CI_LB(i,:)), 'UniformOutput', false)',...
        cellfun(@num2str, num2cell(ORR2_CI_UB(i,:)), 'UniformOutput', false)'];
end
orr2.Properties.VariableNames = {'Response', 'Control', 'Control_CI', 'antiPDL1', 'antiPDL1_CI', 'antiCTLA4', 'antiCTLA_CI', 'antiCTLA4_antiPDL1', 'antiCTLA4_antiPDL1_CI'};
plotSurvival(T2,censor2,Theta2,groups)
subplot(1,2,2)
plotSurvivalFunction(PI2,364,groups)
title('Progresson-free survival at 12 months (kill_{CD8} \propto M^{-0.25})', 'FontSize', 14)

%% Analysing parameter-output relations
corrInOut = plotInputToOutput(Theta1, {'SLD'}, PI1, groups, parameters);
corrInOut2 = plotInputToOutput(Theta2, {'SLD'}, PI2, groups, parameters,'plotOutput', true);
...
save(strjoin({cd '/CIM/HuSim/CIM_ICB_HuSim_Theta2.mat'},''),'PI2')
