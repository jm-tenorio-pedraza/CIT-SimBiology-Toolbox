...
kpro = [repelem({0.5941},12,1);repelem({1.1883},7,1)];
E = [repelem({0.8},12,1);repelem({0.8},7,1)];
...
lb = log([repelem(1e-9,length(PI.data),1)]);
ub = log([repelem(1,length(PI.data),1)]);
...
[phat_anneal, fval_anneal]=anneal(obj_fun,phat_anneal,options_anneal);
...
[x, p_x,accept,pCR] = dreamH(w0,likelihood_fun,prior_fun,...
    size(w0,1),ceil(5e5/size(w0,1)), length(finalValues), 'BurnIn', 2e5,'StepSize',...
    1.38,'H', h);
toc
...
postSamples =x(:,:,12.5e3:300:end);
logP_thinned = p_x(12.5e3:300:end,:);
...
mean(mean(exp(postSamples(:,1:12))))
mean(mean(exp(postSamples(:,13:19))))

%% Save results
save(strjoin({cd 'PI.mat'},'/'),'PI')