...
%     sd_i = SD(group_i);
    time = unique(cell2mat(time_i));          % Count how many time-resolved data points there are for each individual
    ...
    for j=1:length(varindx)                                                 % For each jth variable do:
        ...
        for k=1:n_indiv                                          % For each individual do:
            time_indx_k = ismember(time, time_i{k});
            mat_j(time_indx_k,k) = datavar_j{:,k};                                % assing the observed values in the first t_k rows of the kth column
