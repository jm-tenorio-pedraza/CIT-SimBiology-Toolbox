...
[sim,PI.u]=initializePI(model,parameters,observables,PI,doses, 'MOC1_CTAL4Loop','doseUnits', 'mole');
...
Theta(:,indx_pk) =theta_pk(sampleIndx_pk,:);

% Initial cancer cell number
T0_range= [1 5]; %  0.5 to 5 cm
...
%%
PI = globalSA2(sim,PI,observables,'samples',exp(Theta(1:10,:)),'nsamples',1e3,'time',1:1:PI.tspan(end),...
    'sigma', 1,'inputs', exp(mean(Theta(1:10,:))),'variation', 0.5);
...
%% Save
save('GSA_CIM10_MOC1_PKPD_1_8_kinCD8.mat', 'PI')
