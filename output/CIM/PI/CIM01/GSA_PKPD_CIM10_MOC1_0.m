...
mu=[finalValues(PI.H.PopulationParams(1:end-1)) log(PI.x_0(1))];
sigma=ones(1,length(mu))*.2;
thetaNormal=(randn(10,length(mu))+(mu)).*sigma;

thetaLHS_0=lhsdesign(1e4,length(mu));
lb=exp(mu)*.5;
ub=exp(mu)*2;
thetaLHS=thetaLHS_0.*(ub-lb)+lb;
%%
PI = globalSA2(sim,PI,observables,'samples',(thetaLHS(1:end,:)),'nsamples',1e3,'time',1:1:PI.tspan(end),...
    'sigma', 1,'inputs', (mean(thetaLHS(1:10,:))),'variation', 0.5);
paramRanking = plotPRCC3(PI,PI.paramNames(PI.H.PopulationParams),PI.observablesPlot,'time',...
    1:1:PI.tspan(end),'output', 'mean','kpi', 'sum','nparams',5,'ncol',4);
parameters_hat1 =unique(paramRanking{1:10,:});
%%
save(strjoin({cd 'CIM/PI/CIM01/GSA_CIM10_PKPD_MOC1_0.mat'},'/'), 'PI')
