...
parameters = {'kin_CD8'; 'kin_Treg';'kin_DC';...
    ...
    'K_Tumor'; 'ks_CTLA4_CD8'; 'ks_CTLA4_Treg'; 'KDE_Treg'; 'KDE_MDSC'};
...
PI.H = getHierarchicalStruct3(parameters(1:end-1),PI,'n_sigma', length(observables),...
    'rand_indx', [] , 'cell_indx',[], 'resp_indx', [1],'n_indiv', length(PI.u));
SigmaNames = getVarNames(PI, stateVar);
[beta, sigma_prior] = getVarValues2([.01 .01 .001 .01], [.01 .01 .001 .01], [1 1 1 1], PI);
lb=([1e-2   1e-3   1e-3   1e-4    1e-3    1e-2 1e-5    1e-3 1e-2       1e-2   1e-1    1e-4    1e-4   1e-4    0.01   1e-3     1e-3 1e-3   1e-3    0.01    1e-2    1e-2    1e-3    1e-2    1e-2 1e-3  1e-3])';
ub=([1e2    1e2    1e2    1e2     1e2     2e0  1e1     1    1e2        1e2    1e3     1e3     1e4    1e2     20     2e2      10   1e3    1e3     100     1e2     1e3     10       1e2     1e3 1e2   1e2])';
...
%% Save results
save('PI_CIM30_MOC1_3.mat', 'PI')
if ispc
    load(strjoin({cd 'PI_CIM30_MOC1_2.mat'},'\'),'PI')
