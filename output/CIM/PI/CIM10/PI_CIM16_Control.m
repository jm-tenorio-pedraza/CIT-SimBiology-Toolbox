...
%% Parameter setup
parameters = {'kin_CD8';'kin_DC'; 'kin_Treg';'kill_Treg';'KDE_MDSC'; 'K_IFNg';'K_CTLA4';...
    'K_PDL1';'S_R';'S_L'; 'K_MDSC'; ...
...
PI.H = getHierarchicalStruct(parameters(1:end-1),PI,'n_sigma', length(observables),...
    'rand_indx', [] , 'cell_indx',[12:16], 'n_indiv', length(PI.u));
SigmaNames = getVarNames(PI, stateVar);
[beta, sigma_prior] = getVarValues([1 1 .001], [1, 1 0.001], [1 1 1], PI);

lb=([1e-3    1e-3    1e-3   1e-3    1e-3    1e-3    1e0     1e0     1e-7   1e-7 1e-3   1e-2    1e-2    1e-2   1e-2     1e-3])';
ub=([1e2     1e2     1e2    1e2     1e1     1e2     1e6     1e6     1e0   1e0 1e1    1e2     1e2     1e1    1e2  1e3])';
PI.par = getParamStruct2(sim,PI.H,size(PI.data,1),beta,...
    SigmaNames,'Sigma', sigma_prior, 'ref', 'ones','LB', lb, 'UB', ub);

prior = {'U' 'U' 'U' 'U' 'U' 'U' 'U' 'U' 'U' 'U' 'U' 'U' 'U' 'U' 'U' 'U'};
...
%% Save results
save('PI_CIM16_Control_0.mat', 'PI')
load(strjoin({cd 'PI_CIM10_Control_3.mat'},'/'),'PI')
