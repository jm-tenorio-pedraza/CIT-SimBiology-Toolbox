...
if ispc
    ...
    PI2=getPIData3('\Users\jmten\OneDrive\Dokumente\GitHub\CIT-SimBiology-Toolbox\data\PI_Morisada.mat',...
    stateVar,groups_subset,'output', 'mean','responseGrouping', true, 'kineticGrouping', false);
...
PI.H = getHierarchicalStruct(parameters(1:end-1),PI,'n_sigma', length(observables),...
    'rand_indx', [1 ] , 'cell_indx',[7 8 9 10], 'n_indiv', length(PI.u));
SigmaNames = getVarNames(PI, stateVar);
[beta, sigma_prior] = getVarValues([.1 .1 .01], [.1 .1 .01], [1 1 1], PI);
lb=([1e-3   1e-3    1e-3    1e-4    1e-3   1e-3   1e-4    1e-3    1e-2  1e-5    1e-3    1e-3  1e-3    1e0     1e0     1e0])';
ub=([2e2    1e2     1e2     1e1     1e1    1e2    1e3     1e2     1e2   1e3     1e1     1e5   1e5     1e2     1e7     1e5])';
...
%% Save results
save('PI_CIM5_Control_Reduced_14.mat', 'PI')
if ispc
...
N_i='1';
save(strjoin({cd '/PI_CIM5_Control_14_x_' N_i '.mat'},''), strjoin({'x' N_i},''))
save(strjoin({cd '/PI_CIM5_Control_14_p_x_' N_i '.mat'},''), strjoin({'p_x' N_i},''))
