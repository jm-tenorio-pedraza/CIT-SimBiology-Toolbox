...
residuals_fun=@(p)getNormResiduals(p,@(x)sim(x,100,u,1:1:100),PI,...
    @(x)getPhi2(x,H,size(PI.data,1),x_0),(@(x)getCovariance(x,H)),[]);
...
residuals_fn = @(x) getResiduals(exp(x),@(x)sim(x,100,u,1:1:100),PI,...
    @(x)getPhi2(x,H,length(u),x_0),exp(finalValues(end-length(observables)+1:end)),[]);
